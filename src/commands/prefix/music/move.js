...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    if (!args[0] || !args[1] || isNaN(from) || isNaN(to) || from < 1 || to < 1) {
      return message.reply({ embeds: [buildErrorEmbed('Usage: `!move <from> <to>` — both must be positive numbers.')] });
    }

    const tracks = [...player.queue];

    if (from > tracks.length || to > tracks.length) {
      return message.reply({ embeds: [buildErrorEmbed(`Queue only has **${tracks.length}** track(s). Both positions must be within range.`)] });
    }

    if (from === to) {
      return message.reply({ embeds: [buildErrorEmbed('Source and destination positions are the same.')] });
    }
